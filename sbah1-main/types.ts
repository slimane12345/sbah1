...
export type Page =
  ...
  | 'marketing'
  | 'offers-management'
  | 'home-page-management' // New page for UI management
  | 'ai-algorithms'
  | 'support-chatbot'
  | 'settings'
  // Deprecated pages
...
export interface PastOrder {
  ...
  total: number;
  status: OrderStatus;
  items: { name: TranslatableString | string; quantity: number; category: TranslatableString | string }[];
...
export interface OrderManagementData {
  ...
  items: { name: TranslatableString | string; quantity: number; price: number; options?: (TranslatableString | string)[]; category?: TranslatableString | string }[];
