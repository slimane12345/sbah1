...
const navItems: { id: DriverView; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'my-orders', label: 'طلباتي النشطة', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10l2-2h8a1 1 0 001-1z" /></svg> },
...
const DriverSidebar: React.FC<DriverSidebarProps> = ({ activeView, setActiveView, activeOrderCount }) => {
    return (
        <div className="w-64 bg-white shadow-md flex flex-col">
            <div className="flex items-center justify-center h-20 border-b">
                 <h1 className="text-2xl font-bold text-[#3B82F6]">Sbah Driver</h1>
            </div>
                ...
                {navItems.map(item => (
                    ...
                    >
                        ...
                        {item.id === 'my-orders' && activeOrderCount > 0 && (
                            <span className="bg-white text-[#3B82F6] font-bold text-xs rounded-full h-5 w-5 flex items-center justify-center">
                                {activeOrderCount}
                            </span>
                        )}
                    </button>
