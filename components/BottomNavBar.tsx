        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-[0_-2px_10px_rgba(0,0,0,0.05)] z-30">
            <div className="flex justify-around items-center h-16 max-w-7xl mx-auto">
                {navItems.map((item, index) => {
                    ...
                    return (
                         <NavItem 
                            key={item.id + (item.subPage || '')}
                            label={item.label}
                            icon={item.icon}
                            isActive={activePage === item.id && (item.subPage ? true : true)} // More complex logic needed for subpages
